...
The selected profile provides `salesOrderServerUrl`; localhost remains a safe fallback. Use `--server-url` or the server environment override to target another service. See [Sales Order Service commands](salesorder.md) for context and authentication checks.
